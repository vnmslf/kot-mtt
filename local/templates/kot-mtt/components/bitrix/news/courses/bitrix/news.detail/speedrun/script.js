$(document).ready(function() {
	...
	var fifthSection = $('.fifth__section'),
		seeAll = fifthSection.find('.see-all'),
		counter;
	counter = fifthSection.find('.not-active').length;
	seeAll.text(seeAll.text().replace('{{numbers}}', counter));
	seeAll.on('click', function() {
		fifthSection.find('.intime').removeClass('not-active');
		$(this).hide();
	})
});